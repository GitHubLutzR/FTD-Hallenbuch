...
$selectedGroup = $_GET['filter_group'] ?? ''; // neu
$activeFilter  = '';
$grtable = $hesk_settings['db_hb_pfix'] . 'gruppen'; // neu
...
if ($selectedGroup !== '') {
    ...
    if ((string)$safeGroupId === (string)$selectedGroup) {
    ...
    if ($groupName !== '') {
        $groupCondition = "gruppe LIKE '%" . mysqli_real_escape_string($conn, $groupName) . "%'";
    }
}
// Query
$sql = "SELECT * FROM `{$table}` "
     . ($dateCondition ? $dateCondition . ' ' : '')
     . ($groupCondition ? ($dateCondition ? 'AND ' : 'WHERE ') . $groupCondition . ' ' : '')
     . "ORDER BY datum, von";
...
if (count($eintraege) === 0) {
    ...
    foreach ($eintraege as $e) {
        ...
        $start = $parseTime($von_raw); // neu
        $end   = $parseTime($bis_raw); // neu
        if ($start && $end) { // neu
            if ($end <= $start) { // neu
                $end->modify('+1 day'); // neu
            } // neu
            $totalDurationMinutes += max(0, (int) floor(($end->getTimestamp() - $start->getTimestamp()) / 60)); // neu
        } // neu
    ...
    $totalHours   = intdiv($totalDurationMinutes, 60); // neu
    $totalMinutes = $totalDurationMinutes % 60; // neu
    if ($pdf->GetY() + $lineH + 10 > $pdf->GetPageHeight()) { // neu
        $pdf->AddPage(); // neu
    } // neu
    $pdf->SetFont('Arial', 'B', 10); // neu
    $pdf->Cell(0, $lineH, $conv("Gesamtdauer: {$totalHours} h {$totalMinutes} min"), 1, 1, 'L'); // neu
    $pdf->SetFont('Arial', '', 10); // neu
}
